...
Download all required appliance templates as a [zip file](./appliances.zip). You can also open them one by one in the current directory.
...
In practice, we should change the `HTTP port in the container` and the `HTTP path` in OpenPLC and Scada-LTS.
